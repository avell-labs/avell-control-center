...
export class UtilsService {
    ...
    get pageDisabled(): boolean {
        return this.blurNoInput;
    }
    set pageDisabled(value: boolean) {
    ...
    private languagesMenuArray = [
        ...
        { id: "pt", label: "Portugês", img: "portuguese.svg" },
    ];
    private languageMap;
    ...
    public async openFileDialog(properties): Promise<Buffer> {
        return new Promise<Buffer>((resolve, reject) => {
            this.electron.ipcRenderer
                .invoke("show-open-dialog", properties)
                .then((result) => {
                    if (result.canceled) {
    ...
    public async saveFileDialog(properties): Promise<Buffer> {
        return new Promise<Buffer>((resolve, reject) => {
            this.electron.ipcRenderer
                .invoke("show-save-dialog", properties)
                .then((result) => {
                    if (result.canceled) {
                        reject(result.canceled);
                    } else {
    ...
    public async execFile(command: string): Promise<Buffer> {
        return new Promise<Buffer>((resolve, reject) => {
            this.electron.ipcRenderer
                .invoke("exec-file-async", command)
                .then((result) => {
                    if (result.error === null) {
                        resolve(result.data);
                    } else {
    ...
    public async setBrightnessMode(mode: "light" | "dark" | "system") {
        return await this.electron.ipcRenderer.invoke(
            "set-brightness-mode",
            mode,
        );
    }
    ...
    public async confirmDialog(
        ...
        let result: ConfirmDialogResult = await dialogRef
            .afterClosed()
            .toPromise();
        if (result === undefined) {
            result = {
    ...
    public async choiceDialog(
        config: ChoiceDialogData,
        disableClose: boolean = false,
    ): Promise<ConfirmChoiceResult> {
        ...
        let result: ConfirmChoiceResult = await dialogRef
            .afterClosed()
            .toPromise();
        if (result === undefined) {
            result = {
